...
""", unsafe_allow_html=True) #untuk nyuntikin si css nya


@st.cache_resource #masukin model machine learningnya, cache resource memastikan modelnya hanya di load sekali
def load_model():
    with open('hospital_model.pkl', 'rb') as f: --rb itu read binary jd 10 sesuai yg abis kita encode trs nilainya sbg f
        return pickle.load(f)

bundle        = load_model() #variable utk menampung model dan file dlm picklenya
model         = bundle['model']
scaler        = bundle['scaler'] #elemen2 pendukung aplikasi
...
# ── Hero Header ───────────────────────────────────────────────────────────────
st.markdown(""" #linear gradient utk gradasi bg nya
...
# ── Form ──────────────────────────────────────────────────────────────────────
with st.form("triage_form"): # supaya prioritasin gejala yg urgent jd gak berdasarkan no antrian
    ...
    with col_temp:
        ...
        #index 1 untuk isi defaultnya supaya ga kosong kolomnya
    with col_hr:
